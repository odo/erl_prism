...
maybe_pool(Members, SupervisorName) ->
    ...
    GetInitCall = fun(Member) ->
        case Member#node.proc_info of
            undefined ->
                undefined;
            ProcInfo ->
                proplists:get_value('$initial_call', proplists:get_value(dictionary, ProcInfo))
        end
    end,

    InitialCalls =
    [GetInitCall(Member) || Member <- Members],
