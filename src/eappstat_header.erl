...
handle_call({plot}, _From, State) ->
    ...
    #env{ header = Header, totals = NodeStats, mode = Mode } = State#state.env,
    cecho:werase(Header),
    ...
    {RedsValue, RedsOOM} = eappstat_utils:oom(NodeStats#totals.reductions, 1000),
    eappstat_utils:maybe_hl(Mode, reductions, Header),
    eappstat_utils:f(1,  1, "Reductions: ~.1f ~s/s", [RedsValue, RedsOOM], Header),
    {MemValue, MemOOM} = eappstat_utils:oom(NodeStats#totals.memory, 1024),
    eappstat_utils:maybe_hl(Mode, memory, Header),
    eappstat_utils:f(25, 1, "Memory: ~.1f ~sB", [MemValue, MemOOM], Header),
    {QueueValue, QueueOOM} = eappstat_utils:oom(NodeStats#totals.message_queue_len, 1000),
